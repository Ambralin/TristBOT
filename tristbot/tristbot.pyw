...
REPCHAMPS = []
TIERVALUES = {"IRON": 10000, "BRONZE": 20000, "SILVER": 30000, "GOLD": 40000, "PLATINUM": 50000, "EMERALD": 60000, "DIAMOND": 70000, "MASTER": 80000, "GRANDMASTER": 90000, "CHALLENGER": 100000}
        ...
        with open(r"C:\Users\dzelm\Desktop\bots\tristbot\links.json", 'r+') as f:
            ...
            for name in data['linkedProfiles']:
                if ctx.author.id == name['discordId']:
                    r1 = requests.get("https://" + name["leagueServer"] + ".api.riotgames.com/lol/summoner/v4/summoners/by-puuid/" + name["puuid"] + "?api_key=" + RIOTOKEN)
                    return r1.json()["name"]
    else:
        return user
...
async def guessgame(ctx: discord.ApplicationContext, option: str):
    ...
    if option == "skip" and gamechnl in playingids:
        await ctx.respond("The champion was " + champs[gamechnl].replace("MonkeyKing", "wukong").capitalize())
...
async def spellguess(ctx: discord.ApplicationContext, option: str):
    ...
    if option == "skip":
        ...
        await ctx.respond("The champion was " + spells[gamechnl].replace("MonkeyKing", "wukong").capitalize())
